const fi = (function () {
  return {
    ...
    each: function (collection, callback) {
      ...
      //calling map here would do the same thing but for learning purposes wrote what happens under the hood
    ...
    uniq: function (collection, isSorted = false, callback = false) {
      if (!callback) {
        ...
        for (let e of collection) {
          let newElement = callback(e)
          if (!transformedArray.includes(newElement)) {
            transformedArray.push(callback(newElement))
            uniqArray.push(e)
          }
        }
        return uniqArray
